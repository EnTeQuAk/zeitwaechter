...
void config_load(TimerConfig& cfg) {
    ...
    // Clamp values
    if (cfg.green_minutes < 1 || cfg.green_minutes > 120)
        cfg.green_minutes = defaults.green_minutes;
    if (cfg.yellow_minutes < 1 || cfg.yellow_minutes > 120)
        cfg.yellow_minutes = defaults.yellow_minutes;
    if (cfg.final_minutes < 1 || cfg.final_minutes > 120)
        cfg.final_minutes = defaults.final_minutes;
}
